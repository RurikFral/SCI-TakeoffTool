...
class EyebrowCalculator:
    def __init__ (self, root):
        #variables
        self.radius = StringVar()
        self.proAmount = StringVar()
        ...
        #row1
        Label(self.frame, text = "Proportional Amount").grid(row = 1, column = 0, sticky = E, padx = (5,5), pady = (5,5))

        self.proAmountEntry = Entry(self.frame, textvariable = self.proAmount, width = 20)
        self.proAmountEntry.grid(row = 1, column = 1)

        Label(self.frame, text = "(Typically 4)").grid(row = 1, column = 2, sticky = W, padx = (5,5), pady = (5,5))
    ...
    def calculateTotal(self, event=None): 
        ...
        proAmountFloat = self.helperService.safe_cast(self.proAmount.get(), float, 0)
        ...
        try:
            self.totalNumberOfBars.set(str(int(round((8*radiusFloat*radiusFloat)/(proAmountFloat*centerToCenterFloat*(defaultBarLength-overlapFloat))))))        
